...
def _postproc_affimg(cur_color, cur_depth, cur_afford):
    ...
    cur_afford[np.where(normalBasedSuctionScores < 0.2)] = 0 
    print(normalBasedSuctionScores)
...
if __name__ == "__main__":
    ...
    while(cmd != 'n'):
        ...
        print(np.transpose(hand_calcu))

        cmd = input("go? [y/n]")
        if cmd == 'y':
            add_height = 0.15
            pos_in = [hand_calcu[0][0],hand_calcu[1][0],0.65,0,-3.139,0] # make the hand vertical down and adjust z of xyz
            robot.movel(pos_in, acc = robot_acc,vel = robot_vel)
            pos_in = [hand_calcu[0][0],hand_calcu[1][0],hand_calcu[2][0]+hand_height-0.063,0,-3.139,0] # make the hand vertical down and adjust z of xyz
            robot.movel(pos_in, acc = robot_acc,vel = robot_vel) 
