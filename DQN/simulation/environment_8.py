...
class DQNEnvironment(object):
    ...
    def calc_reward(self):
        ...
        self.metric = self.reward_metric(self.local_afford_new)
        # TODO:
        if (self.metric - last_metric) > 0.02:
            return 1.
        else:
            if (self.metric - last_metric) < -0.02:
                return 0.# -0.1? it seems the positive reward is too little
            else:
    ...
    def reward_metric(self, afford_map):
        ...
        self.metric_all.append(reward_metric)
    ...
    def action2ur5(self, action):
        ...
        """
        '''
        # 8, 8, 16 is the output of the u-net
        idx = np.unravel_index(action, (8, 8, 16))
        ...
        '''
        # 8, 8, 8 is the output of the u-net
        idx = np.unravel_index(action, (8, 8, 8))
        relate_local = list(idx[0:2])
        ori = idx[2] * 360. / 8.
        push_depth = 0.045
        ...
        relate_local = (np.asarray(relate_local) + 1.0) * self.screen_height / 8 - 1.0
