...
class Agent(BaseModel):
    ...
    def train(self):
        ...
        for self.step in tqdm(range(self.start_step, self.max_step), ncols=70, initial=self.start_step):
            ...
            # 2. act
            # notice the action is in [0, 8*8*8-1]
    ...
    def q_learning_mini_batch(self):
        ...
        # q_t_plus_l = batch_size * self.action_num(8*8*8)
    ...
    def _build_dqn(self):
        ...
        with tf.variable_scope('prediction'):
            ...
            for i in range(8):
                ...
                # ->output
                self.q_1, self.w['q_1_w'], self.w['q_1_b'] = conv2d(self.l9,
                    1, [3, 3], [1, 1], initializer, activation_fn, self.cnn_format, name=idx +'_lq_1')
                # q_1 = None*16*16*1
                self.q = max_pool(self.q_1, [2, 2], [2, 2], self.cnn_format, name=idx +'_lq')
                # q = None*8*8*1
        ...
        with tf.variable_scope('target'):
            ...
            # s_t = None*32*32*8(history_length*inChannel)
            for i in range(8):
                idx = str(i)
                # downsample_1
                self.t_w = {}
                ...
                # ->output
                self.target_q_1, self.t_w['q_1_w'], self.t_w['q_1_b'] = conv2d(self.target_l9,
                    1, [3, 3], [1, 1], initializer, activation_fn, self.cnn_format, name=idx +'_target_lq_1')
                # q_1 = None*16*16*1
                self.target_q = max_pool(self.target_q_1, [2, 2], [2, 2], self.cnn_format, name=idx +'_target_lq')
                # q = None*8*8*1
                if i == 0:
    ...
    def play(self, n_step=40, n_episode=27, remove_obj_num = 5, test_ep=None):
        """
    ...
    def randomplay(self, n_step=40, n_episode=20, remove_obj_num = 5):
        """
    ...
    def exp_play(self, remove_obj_num = 4, test_ep=None, use_dqn = True):
        ...
        if use_dqn:
            ...
            while True:
                if terminal or signal_push == 1:
                    # apply the suck action
                    input('\n !>!>!>!>!>! Continue to suck the objects \n')
                    self.env.ope() #suck action
