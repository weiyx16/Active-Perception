...
class UR5(object):
    ...
    def video_used(self):
        self.posori1 = [-0.109,0.257,0.582,0.005,-3.113,0.005]
        self.possafe1 = [-0.003,0.477,0.636,0.10,-3.05,-0.05]
        self.positem1 = [0.083,0.519,0.43,2.26,2.1,-0.07]
        self.positem2 = [0.145,0.363,0.439,0.008,3.139,-0.166]
        self.positem3 = [-0.008,0.436,0.506,0.003,3.14,0.62]
        self.positem4 = [-0.222,0.424,0.490,1.85,2.66,0.008]
        self.posloose = [-0.431,-0.018,0.556,2.21,2.36,0.04]

        input('go?')
        self.robot.movel(self.posori1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.positem1, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.grasp()
        time.sleep(7)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posloose, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.handopen()
        time.sleep(1)
        self.hand.place()
        time.sleep(1)
        ...
        input('go?')
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.positem2, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.grasp()
        time.sleep(7)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posori1, acc = self.robot_acc,vel = self.robot_vel)
        ...
        input('go?')
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.positem3, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.grasp()
        time.sleep(7)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posloose, acc = self.robot_acc,vel = self.robot_vel)
        ...
        input('go?')
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.positem4, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.grasp()
        time.sleep(7)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posori1, acc = self.robot_acc,vel = self.robot_vel)
    ...
    def video_act_perc(self):
        self.posori1 = [-0.109,0.257,0.582,0.005,-3.113,0.005]
        self.possafe1 = [-0.003,0.477,0.636,0.10,-3.05,-0.05]
        self.posact1 = [0.05,0.415,0.438,2,2.5,0.1]
        self.posact2 = [-0.05,0.415,0.438,2,2.5,0.1]
        self.posact3 = [0.08,0.425,0.427,1.67,2.69,0.13]
        self.posact4 = [0.18,0.455,0.427,1.67,2.69,0.13]
        self.posact5 = [0.09,0.39,0.44,0.9,2.88,0.04]
        self.posloose = [-0.431,-0.018,0.556,2.21,2.36,0.04]
        ...
        #self.hand.handin()
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posact1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posact2, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posori1, acc = self.robot_acc,vel = self.robot_vel)
        ...
        #self.hand.handin()
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posact3, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posact4, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posori1, acc = self.robot_acc,vel = self.robot_vel)
        
        input('go?')
        self.robot.movel(self.possafe1, acc = self.robot_acc,vel = self.robot_vel)
        self.robot.movel(self.posact5, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.handopen()
        time.sleep(0.5)
        self.hand.grasp()
        time.sleep(7)
