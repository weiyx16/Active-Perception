...
class Camera(object):
    ...
    def __init__(self, Lua_PATH):
        ...
        self.Img_WIDTH = 512
        self.Img_HEIGHT = 424
        self.border_pos = [0, self.Img_HEIGHT - 1, 0, self.Img_WIDTH - 1] # TODO:
    ...
    def get_camera_data(self):
        ...
        registered_lr = np.fliplr(self.registered.asarray(np.uint8))
        registered_lr = registered_lr[:,:,0:3] # Remove the alpha channel

        depth = np.fliplr(depth.asarray(np.uint32))
        # depth image always crushed with ir
        while np.mean(depth) > 2000: # TODO:
            time.sleep(0.05) 
            frames = self.listener.waitForNewFrame()
        ...
        self.device.stop()
        self.device.close()
        return depth, registered_lr

    def save_image(self, cur_depth, cur_color):
        """
            -- Save Color&Depth images
        """
        img = Image.fromarray(cur_color.astype(np.uint8)).convert('RGB')
    ...
    def local_patch(self, patch_size = (128, 128)):
        """
        ...
        cur_depth_path, cur_img_path = self.save_image(self.cur_depth, self.cur_color)
        # feed this image into affordance map network and get the h5 file
        cur_res_path = os.path.join(self.Save_PATH, '_results.h5')
    ...
    def pixel2ur5(self, u, v, push_depth, depth = 0.0, is_dst = True):
        """
        ...
        location_3d = [x, y, depth]

        location_3d_homo = [[location_3d[0]], [location_3d[1]], [location_3d[2]],[1]]
        return np.dot(np.linalg.inv(self.tran), location_3d_homo), depth
...
class UR5(object):
    ...
    def __init__(self):
        ...
        self.robot_vel = 0.5    #accerlation
        self.robot_acc = 0.3 
        self.hand_height = 0.4 # mile
        self.pos_ori = [-0.1,0.06,0.64,0,-3.139,0]
    ...
    def ur5push(self, move_begin, move_to):
        ...
        """
        # push act
        pos_in = [move_begin[0],move_begin[1],self.safe_height,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)    
        self.hand.handin()
        pos_in = [move_begin[0],move_begin[1],move_begin[2]+self.hand_height-0.063,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)
        # TODO:0.063
        pos_in = [move_to[0],move_to[1],move_to[2]+self.hand_height-0.063,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)
        pos_in = [move_to[0],move_to[1],self.safe_height,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)
        self.hand.handopen()
        # return to the initial pose
        self.robot.movel(self.pos_ori, acc = self.robot_acc,vel = self.robot_vel)

    def ur5suck(self, suck_location):
        """
            Suck in the fixed location
        """
        pos_in = [suck_location[0],suck_location[1],self.safe_height,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)  
        pos_in = [suck_location[0],suck_location[1],suck_location[2]+self.hand_height-0.063,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)        
        self.hand.suck()
        pos_in = [suck_location[0],suck_location[1],self.safe_height,0,-(math.pi-0.2),0] # make the hand vertical down and adjust z of xyz
        self.robot.movel(pos_in, acc = self.robot_acc,vel = self.robot_vel)
...
class REALEnvironment(object):
    ...
    def new_scene(self):
        ...
        self.screen, self.local_afford_past, self.location_2d = self.camera.local_patch((self.screen_height, self.screen_width))
        ...
        return self.screen, 0., -1, False #self.terminal TODO:
    ...
    def action2ur5(self, action):
        ...
        move_begin, src_depth = self.camera.pixel2ur5(real_local[0], real_local[1], push_depth, is_dst = False)
        move_to, _ = self.camera.pixel2ur5(real_dest[0], real_dest[1],  push_depth, src_depth, is_dst = True)
    ...
    def ope(self):
        ...
        location_3d, _ = self.camera.pixel2ur5(self.location_2d[0], self.location_2d[1], 0., is_dst = True)
        # push act
        self.ur5.ur5suck(location_3d)

    def observe_screen(self):
        _, _, self.location_2d = self.camera.local_patch((self.screen_height, self.screen_width))
